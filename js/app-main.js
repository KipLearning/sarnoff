...
mainCtlr.$inject = ['$scope', '_'];

function mainCtlr($scope, _){
...
<p>Cozzolino holds a bachelor's degree in electrical engineering from University of Massachusetts Dartmouth and a master's degree in business administration from Anna Maria College.</p>"}},
	...
	$scope.toggleYear = function(year) {
		if ($scope.yearShown == year) {
			$scope.yearShown = "";
		}
		else 
		{
			$scope.yearShown = year;
		}
	}
	$scope.summits = [
			...
			{url: "dresden16/", city:"Dresden, Germany", sortDate:"1475121600000", date:"Thursday, September 29, 2016"},
			...
			{url: "casablanca/", city:"Casablanca, Morocco", sortDate:"1494475200000", date:"Thursday, May 11, 2017"},
		...
		_($scope.summits).each(function(s){
				s.timeTill = s.sortDate >= $scope.currentDate ? (s.sortDate - $scope.currentDate) : s.sortDate;
				s.year = (new Date(+s.sortDate)).getFullYear();
				$scope.yearlySummits[s.year]=$scope.yearlySummits[s.year] || [];
				$scope.yearlySummits[s.year].push(s);
			});
